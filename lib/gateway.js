...
function logCurl(fullPath, init, token) {
  const method = (init.method || "GET").toUpperCase();
  const bodyArg = init.body ? ` \\\n  -d '${init.body}'` : "";
  const ctArg = (init.headers?.["Content-Type"] || init.headers?.["content-type"])
    ? ` \\\n  -H "Content-Type: ${init.headers["Content-Type"] || init.headers["content-type"]}"`
    : "";
  console.log(
    `\n[gateway-curl]\ncurl --max-time 60 -sS -X ${method} "${fullPath}" \\\n  -H "Authorization: Bearer ${token}"${ctArg}${bodyArg} | jq\n`
  );
}
